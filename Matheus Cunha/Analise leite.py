...
from sklearn.model_selection import train_test_split #divide o modelo em modelo de teste e treino.
import pickle #para salvar os modelos já treinados
...
with open('C:/Users/mathe/Documents/Estudos Python/df_machine_learn.sav', 'wb') as f:
   
arq_1 = open('df_machine_learn.sav', 'wb')
pickle.dump(modelo_1,arq_1)
arq_1.close()
arq_1 = open('df_machine_learn.sav', 'rb')

dados_de_teste_e_treino = pickle.load(arq_1)
#salvar os modelos de machine learn
...
df_0 = df.loc[df['Grade']==0]
df_1 = df.loc[df['Grade']==1]
df_2 = df.loc[df['Grade']==2]

from scipy import stats

#teste de distribuição nas variáveis da tabela.

stats.shapiro(df['pH'])
stats.shapiro(df_0['pH'])
stats.shapiro(df_1['pH'])
stats.shapiro(df_2['pH'])

plt.figure()
sns.displot(df['pH'], kind = 'kde', color = 'black')
sns.displot(df_0['pH'], kind = 'kde', color = 'blue')
sns.displot(df_1['pH'], kind = 'kde', color = 'red')
sns.displot(df_2['pH'], kind = 'kde', color = 'green')
...
#teste de distribuição nos modelos de machine learn

stats.shapiro(scores_1)
sns.displot(scores_1, kind = 'kde')

stats.shapiro(scores_2)
sns.displot(scores_2, kind = 'kde')

stats.shapiro(scores_3)
sns.displot(scores_3, kind = 'kde')

stats.shapiro(scores_4 )
sns.displot(scores_4, kind = 'kde')

# dividir os grades nos graficos.

#%% teste de hípoteses 
previsoes_proba_1 = modelo_1.predict_proba(x_test)
df_proba =pd.DataFrame(previsoes_proba_1)
